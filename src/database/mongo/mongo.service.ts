...
export class MongoService {
  ...
  async findDocuments<T extends Document>(model: Model<T>, condition: FilterQuery<T>, projection?: string): Promise<T[]> {
    return model.find(condition, projection).exec();
  }

  async updateEncryptedDocuments<T extends Document>(model: Model<T>, condition: FilterQuery<T>, setObjectParams: Record<string, any>): Promise<MongoUpdateResponse> {
    ...
    if (result.matchedCount > 0) {
      const updatePromises = documents.map(async (document) => {
        for (const [key, value] of Object.entries(setObjectParams)) {
  ...
  async updateManyDocuments<T extends Document>(model: Model<T>, condition: FilterQuery<T>, setObjectParams: Record<string, any>): Promise<MongoUpdateResponse> {
    return model.updateMany(condition, { $set: setObjectParams }).lean().exec();
  }
}
