...
import { IMasterCustomerCondition } from 'src/common/interfaces/database_domain.interface';
...
export class QueryMasterCustomerService {
    ...
    async queryMasterCustomer(queryMasterCustomerDto: QueryMasterCustomerDto) {
        const condition: IMasterCustomerCondition = {}
        ...
        if(queryMasterCustomerDto.SAPCode) {condition.SAPCode = queryMasterCustomerDto.SAPCode}
        else if(queryMasterCustomerDto.customerName) {condition.customerName = queryMasterCustomerDto.customerName}
        ...
        if (queryMasterCustomerDto.flagValidate) {condition.status = STATUS_INACTIVE}
        if (queryMasterCustomerDto.products) {condition.products = queryMasterCustomerDto.products}
