      ...
      response.result = await this.kafkaService.createConsumer(this.kafkaConfig, topics, async (message) => {
        ...
        if (message.topic === 'sap.proxy.trackingStatusUpdated') {
          ...
          if (jsonContent.body.Item && jsonContent.body.Item.length > 0) {
            ...
            setParamsOptimusOrder.modifyDateTime = dayjs
              .tz(Date.now(), TIMEZONE_THAI)
              .format('YYYY-MM-DDTHH:mm:ss.SSS[Z]');
            const result = await this.mongoService.updateEncryptedDocuments(
              this.OptimusOrdersModel,
              conditionOptimusOrder,
            ...
            if (result.matchedCount === 1) {
              ...
              this.mongoService.updateEncryptedDocuments(
                this.OptimusOmniOrdersModel,
                conditionOptimusOmniOrder,
                setParamsOptimusOmniOrder,
              );
            }
          }
          ...
          if (
            Array.isArray(
              result['n0:BusinessPartnerSUITEBulkReplicateRequest'].BusinessPartnerSUITEReplicateRequestMessage,
            )
          ) {
            businessPartnerSRRM =
              result['n0:BusinessPartnerSUITEBulkReplicateRequest'].BusinessPartnerSUITEReplicateRequestMessage;
            ...
            for (const item of businessPartnerSRRM) {
              ...
              if (bpGroup && validBPGroups.includes(bpGroup)) {
                ...
                if (setParamsMasterCustomer.categoryCode === 1) {
                  ...
                  setParamsMasterCustomer.customerName =
                    `${name.FormOfAddressCode} ${name.GivenName} ${name.FamilyName}`.trim();
                } else if (setParamsMasterCustomer.categoryCode === 2) {
                  const name = item.BusinessPartner.Common.Organisation.Name;
                  setParamsMasterCustomer.customerName =
                    `${name.FirstLineName} ${name.SecondLineName} ${name.ThirdLineName} ${name.FourthLineName}`.trim();
                }
                ...
                this.mongoService.updateEncryptedDocuments(
                  this.MasterCustomersModel,
                  consditionMasterCustomer,
                  setParamsMasterCustomer,
                );
              }
            }
          }
